...
from .constants import PAYMENT_METHODS, DELIVERY_METHODS



class Order(models.Model):
    payment_method = models.CharField(max_length=10, choices=PAYMENT_METHODS, verbose_name='Способ оплаты')
    delivery_method = models.CharField(max_length=10, choices=DELIVERY_METHODS, verbose_name='Способ доставки')
...
class FlowerOrder(models.Model):
    order = models.ForeignKey(Order, verbose_name='Заказ', related_name='order', on_delete=models.CASCADE)
    flower = models.ForeignKey(Flower, verbose_name='Цветок', related_name='flower', on_delete=models.CASCADE)
...
class GoodsOrder(models.Model):
    order = models.ForeignKey(Order, verbose_name='Заказ', related_name='good_order', on_delete=models.CASCADE)
    good = models.ForeignKey(Good, verbose_name='Товар', related_name='good', on_delete=models.CASCADE)
    price = models.DecimalField(max_digits=10, decimal_places=2, verbose_name='Цена')
    quantity = models.PositiveIntegerField(verbose_name='Количество')
    total = models.PositiveIntegerField(verbose_name='Итого')
